...
class SparkG5kConf:
    ...
    def __init__(self, site: str, cluster: str, worker: int, jobname: str=_DEFAULT_JOB_NAME, time: str=_DEFAULT_TIME, start: str=_DEFAULT_START):
        self.__site = site
    ...
    def start(self):
        ...
        if self.__start_time == _DEFAULT_START:
            ...
            conf = (
                G5kConf.from_settings(job_type="allow_classic_ssh", job_name="Test_java_version", walltime=self.__time, reservation=self.__start_time)
                .add_network_conf(my_network)
                .add_machine(
                    roles=[_ROLE_MASTER], cluster=self.__cluster, nodes=1, primary_network=my_network
                )
    ...
    def submitWithLog(self, path_jar: str, classname: str, spark_args: Dict={}, java_args: Dict={}, path_log: str="/tmp/out.log", path_err: str="/tmp/out.err"):
