...
my_job = "Relational2Class_d2_byrule"
...
parameters = dict(nb_worker_cores=[(1,4), (2,4), (4,4), (8,4), (8,8), (16,8)], sleep=[0,50,120,250,500,1000,2000], size=[20])
        ...
        try:
            p.shell(
                "$SPARK_HOME/bin/spark-submit \
                    ...
                    -partition " + str(nb_partition)+ " -sleep_instantiate " +str(sleeping)+" -sleep_guard " + str(sleeping) +" \
		            -sleep_apply " + str(sleeping) + " -mode simple -step " + str(nstep)+ " \
                    >> " + path_log + " 2>> " + path_err
            )
...
# start the machines
    ...
    with play_on(pattern_hosts="master", roles=roles, run_as=username) as p:
        ...
        p.shell(
            "echo \" pivot,element,link,executor,core,partition,sleeping_guard,sleeping_instantiate,sleeping_apply,total_time,time_tuples,time_instantiate,time_extract,time_broadcast,time_apply \" > " + path_log
        )
        p.shell(
