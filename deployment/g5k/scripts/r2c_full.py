...
conf = (
    G5kConf.from_settings(job_type="allow_classic_ssh", job_name="SPARK_r2c_full", reservation="2021-06-08 19:00:00", walltime="13:00:00")
    # G5kConf.from_settings(job_type="allow_classic_ssh", job_name="SPARK_r2c_dumb", walltime="02:00:00")
...
def bench(parameter, master, roles, username):
    ...
    path_log = "/tmp/r2c_full.log"
    path_err = "/tmp/r2c_full.err"
        ...
        try:
            p.shell(
                "/home/"+username+"/Software/spark-3.1.1-bin-hadoop2.7/bin/spark-submit \
                    ...
                    --class org.atlanmod.Main_Relational2Class_Full \
                    /home/jphilippe/jars/SparkTE-1.0-SNAPSHOT.jar -core "+str(nb_core)+" -executor "+str(nb_worker)+" -size " + str(size) + " -mode dumb -sleep 4 \
                    >> " + path_log + " 2>> " + path_err
            )
...
# Run the job
    ...
    for test in range(30):
        ...
        sweeper = ParamSweeper(
            persistence_dir=str(Path("sweeps_r2c_full_"+str(test))), sweeps=sweeps, save_sweeps=True
        )
