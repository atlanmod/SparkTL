...
conf = (
    G5kConf.from_settings(job_type="allow_classic_ssh", job_name="SPARK_cartesian", reservation="2021-06-11 16:45:00", walltime="02:00:00")
...
def bench(parameter, master, roles, username):
    ...
    path_log = "/tmp/cartesian.log"
    path_err = "/tmp/cartesian.err"
        ...
        try:
            p.shell(
                "/home/"+username+"/Software/spark-3.1.1-bin-hadoop2.7/bin/spark-submit \
                    ...
                    --class org.atlanmod.Main_cartesian \
                    /home/jphilippe/jars/SparkTE-1.0-SNAPSHOT.jar -core "+str(nb_core)+" -executor "+str(nb_worker)+" -size " + str(size) + " -arity " + str(arity) + " \
                    >> " + path_log + " 2>> " + path_err
            )
...
# Run the job
    ...
    for test in range(10):
        ...
        sweeper = ParamSweeper(
            persistence_dir=str(Path("sweeps_cartesian_"+str(test))), sweeps=sweeps, save_sweeps=True
        )
