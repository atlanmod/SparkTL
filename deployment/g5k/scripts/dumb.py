...
conf = (
    # G5kConf.from_settings(job_type="allow_classic_ssh", job_name="SPARK_sleeping", reservation="2021-05-26 21:30:00", walltime="10:00:00")
    G5kConf.from_settings(job_type="allow_classic_ssh", job_name="SPARK_dumb", reservation="2021-05-26 10:30:00", walltime="02:00:00")
...
def bench(parameter, master, roles, username):
    ...
    path_log = "/tmp/sleep.log"
    path_err = "/tmp/sleep.err"
        ...
        try:
            p.shell(
                "/home/"+username+"/Software/spark-3.1.1-bin-hadoop2.7/bin/spark-submit \
                    ...
                    --class org.atlanmod.Main_sleep \
                    /home/jphilippe/jars/SparkTE-1.0-SNAPSHOT.jar -core "+str(nb_core)+" -worker "+str(nb_worker)+ " -size 1800 \
                    >> " + path_log + " 2>> " + path_err
            )
...
# Run the job
    ...
    for test in range(30):
        ...
        sweeper = ParamSweeper(
            persistence_dir=str(Path("sweeps_dumb_"+str(test))), sweeps=sweeps, save_sweeps=True
        )
