...
conf = (
    # G5kConf.from_settings(job_type="allow_classic_ssh", job_name="SPARK_relational2class_simple", reservation="2021-05-11 19:00:00", walltime="13:00:00")
    G5kConf.from_settings(job_type="allow_classic_ssh", job_name="SPARK_relational2class_simple", walltime="2:00:00")
...
def bench(parameter, master, roles, username, ntest):
    ...
    path_log = "/tmp/sparkte_r2c_HM.log"
    path_err = "/tmp/sparkte_r2c_HM.err"
        ...
        try:
            p.shell(
                "/home/"+username+"/Software/spark-3.1.1-bin-hadoop2.7/bin/spark-submit \
                    ...
                    " --size "+str(size)+" --ntests 1 -csv --path " + path + " --mode par \
                    >> " + path_log + " 2>> " + path_err
            )
            os.system("rm -rf /home/jphilippe/Software/spark-3.1.1-bin-hadoop2.7/work")
        # p.shell(
