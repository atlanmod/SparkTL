...
class TestNewClass2Relational extends AnyFunSuite {
...
//            person_emailAddresses_id__to__person_emailAddresses, person_emailAddresses_type__to__person_emailAddresses,
    ...
    test ("test class2relational default parameters") {
        val config = new Parameters.ConfigEngine()
        val conf = new SparkConf().setAppName("Test").setMaster("local")
        val c2r = Class2Relational.class2relational(ClassMetamodelNaive, RelationalMetamodelNaive)
        val res = TransformationEngine.execute(c2r, classmodel, ClassMetamodelNaive.metamodel, config, 4, SparkContext.getOrCreate(conf))
