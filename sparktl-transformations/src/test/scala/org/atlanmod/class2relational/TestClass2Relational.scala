...
import org.atlanmod.class2relational.model.classmodel.element.{ClassAttribute, ClassClass, ClassDatatype, ClassClassifier}
...
import org.atlanmod.class2relational.model.classmodel.{ClassElement, ClassModel}
...
class TestClass2Relational extends AnyFunSuite {
...
//            person_emailAddresses_id__to__person_emailAddresses, person_emailAddresses_type__to__person_emailAddresses,
    ...
    test("test getClassAttributes naive"){
        var res = ClassMetamodelNaive.getClassAttributes(family, classmodel).getOrElse(List())
        var exp = List(family_name, family_members)
        assert(ListUtils.weak_eqList(res, exp))
        res = ClassMetamodelNaive.getClassAttributes(person, classmodel).getOrElse(List())
        exp = List(person_closestFriend, person_firstName, person_emailAddresses)
    }

    test("test getClassAttributes with map"){
        var res = ClassMetamodelWithMap.getClassAttributes(family, classmodel).getOrElse(List())
        var exp = List(family_name, family_members)
        assert(ListUtils.weak_eqList(res, exp))
        res = ClassMetamodelWithMap.getClassAttributes(person, classmodel).getOrElse(List())
        exp = List(person_closestFriend, person_firstName, person_emailAddresses)
    }

    test("test getAttributeType naive"){
        var res = ClassMetamodelNaive.getAttributeType(family_name, classmodel).toList
        var exp : List[ClassClassifier] = List(type_string)
        assert(ListUtils.weak_eqList(res, exp))
        res  = ClassMetamodelNaive.getAttributeType(family_members, classmodel).toList
        exp = List(person)
        assert(ListUtils.weak_eqList(res, exp))
        res  = ClassMetamodelNaive.getAttributeType(person_firstName, classmodel).toList
        exp = List(type_string)
        assert(ListUtils.weak_eqList(res, exp))
        res  = ClassMetamodelNaive.getAttributeType(person_emailAddresses, classmodel).toList
        exp = List(type_string)
        assert(ListUtils.weak_eqList(res, exp))
        res  = ClassMetamodelNaive.getAttributeType(person_closestFriend, classmodel).toList
        exp = List(person)
        assert(ListUtils.weak_eqList(res, exp))
    }

    test("test getAttributeType with map"){
        var res = ClassMetamodelWithMap.getAttributeType(family_name, classmodel).toList
        var exp : List[ClassClassifier] = List(type_string)
        assert(ListUtils.weak_eqList(res, exp))
        res  = ClassMetamodelWithMap.getAttributeType(family_members, classmodel).toList
        exp = List(person)
        assert(ListUtils.weak_eqList(res, exp))
        res  = ClassMetamodelWithMap.getAttributeType(person_firstName, classmodel).toList
        exp = List(type_string)
        assert(ListUtils.weak_eqList(res, exp))
        res  = ClassMetamodelWithMap.getAttributeType(person_emailAddresses, classmodel).toList
        exp = List(type_string)
        assert(ListUtils.weak_eqList(res, exp))
        res  = ClassMetamodelWithMap.getAttributeType(person_closestFriend, classmodel).toList
        exp = List(person)
        assert(ListUtils.weak_eqList(res, exp))
    }

    test("test getAttributeOwner naive"){
        var res = ClassMetamodelNaive.getAttributeOwner(family_name, classmodel).toList
        var exp = List(family)
        assert(ListUtils.weak_eqList(res, exp))
        res = ClassMetamodelNaive.getAttributeOwner(family_members, classmodel).toList
        exp = List(family)
        assert(ListUtils.weak_eqList(res, exp))
        res = ClassMetamodelNaive.getAttributeOwner(person_firstName, classmodel).toList
        exp = List(person)
        assert(ListUtils.weak_eqList(res, exp))
        res = ClassMetamodelNaive.getAttributeOwner(person_closestFriend, classmodel).toList
        exp = List(person)
        assert(ListUtils.weak_eqList(res, exp))
        res = ClassMetamodelNaive.getAttributeOwner(person_emailAddresses, classmodel).toList
        exp = List(person)
        assert(ListUtils.weak_eqList(res, exp))
    }

    test("test getAttributeOwner with map"){
        var res = ClassMetamodelWithMap.getAttributeOwner(family_name, classmodel).toList
        var exp = List(family)
        assert(ListUtils.weak_eqList(res, exp))
        res = ClassMetamodelWithMap.getAttributeOwner(family_members, classmodel).toList
        exp = List(family)
        assert(ListUtils.weak_eqList(res, exp))
        res = ClassMetamodelWithMap.getAttributeOwner(person_firstName, classmodel).toList
        exp = List(person)
        assert(ListUtils.weak_eqList(res, exp))
        res = ClassMetamodelWithMap.getAttributeOwner(person_closestFriend, classmodel).toList
        exp = List(person)
        assert(ListUtils.weak_eqList(res, exp))
        res = ClassMetamodelWithMap.getAttributeOwner(person_emailAddresses, classmodel).toList
        exp = List(person)
        assert(ListUtils.weak_eqList(res, exp))
    }

    test ("test class2relational naive") {
        val c2r = Class2Relational.class2relational(ClassMetamodelNaive, RelationalMetamodelNaive)
        val res = TransformationSequential.execute(c2r, classmodel, ClassMetamodelNaive.metamodel)
        val res_model = new RelationalModel(res.allModelElements.asInstanceOf[List[RelationalElement]],res.allModelLinks.asInstanceOf[List[RelationalLink]])
        val exp = getRelationalModelSample
        assert(res_model.weak_equals(exp))
    }

    test ("test class2relational with map") {
        val c2r = Class2Relational.class2relational(ClassMetamodelWithMap, RelationalMetamodelWithMap)
        val res = TransformationSequential.execute(c2r, classmodel, ClassMetamodelWithMap.metamodel)
        val res_model = new RelationalModel(res.allModelElements.asInstanceOf[List[RelationalElement]],res.allModelLinks.asInstanceOf[List[RelationalLink]])
        val exp = getRelationalModelSample
        assert(res_model.weak_equals(exp))
    }
