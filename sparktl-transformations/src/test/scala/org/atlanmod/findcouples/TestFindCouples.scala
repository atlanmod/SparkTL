...
class TestFindCouples  extends AnyFunSuite {
    ...
    test("getPersonsOfMovieUsingMap"){
        MovieMetamodelWithMap.getPersonsOfMovie(StarWarsModel, return_of_the_jedi) match {
            case Some(actors) =>
                assert(actors.size == 4)
                assert(actors.contains(mark_hamill))
                assert(actors.contains(carrie_fisher))
    ...
    test("getMoviesOfPersonUsingMap"){
        MovieMetamodelWithMap.getMoviesOfPerson(StarWarsModel, mark_hamill) match {
            case Some(movies) =>
                assert(movies.size == 3)
                assert(movies.contains(a_new_hope))
                assert(movies.contains(the_empire_strikes_back))
