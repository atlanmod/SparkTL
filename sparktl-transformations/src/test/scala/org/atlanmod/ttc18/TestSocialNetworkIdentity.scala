...
class TestSocialNetworkIdentity extends AnyFunSuite{
    ...
    test("test identity with naive metamodel"){
        val mymodel = model
        val metamodel = SocialNetworkMetamodelNaive
        val tr = org.atlanmod.ttc18.transformation.Score.score(metamodel)
        val res = TransformationSequential.execute(tr, mymodel, metamodel.metamodel)
        assert(ListUtils.eqList(res.allModelElements, mymodel.allModelElements))
        assert(ListUtils.eqList(res.allModelLinks, mymodel.allModelLinks))
    }

    test("test identity with optimal metamodel"){
        val mymodel = model
        val metamodel = SocialNetworkMetamodelWithMap
        val tr = org.atlanmod.ttc18.transformation.Score.score(metamodel)
        val res = TransformationSequential.execute(tr, mymodel, metamodel.metamodel)
        assert(ListUtils.eqList(res.allModelElements, mymodel.allModelElements))
        assert(ListUtils.eqList(res.allModelLinks, mymodel.allModelLinks))
