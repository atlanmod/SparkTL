...
data <- read.csv(file="x4_raw_byrule.csv", colClasses=colTypes, comment.char = "#")
```
...
ggplot(data=df.with_speedups, aes(x=total_cores,y=total_time.speedup,group=as.factor(sleeping_time),color=as.factor(sleeping_time))) + 
  geom_line() + geom_point() + labs(fill = "") + geom_segment(aes(x = 4, y = 1, xend = 128, yend = 32),  color="black", linetype="dashed") + 
  geom_segment(aes(x = 4, y = 1, xend = 128, yend = 16),  color="red", linetype="dashed") +
  theme(legend.key.size = unit(0.5, 'cm')) + theme(legend.position = "none") + xlab("number of cores") + ylab("relative speedup") + scale_color_discrete(name="Introduced simulated computation\n time on phases (ms)") +
...
ggsave("x4_sleeping.pdf",width = 4.1, height = 3)
```
