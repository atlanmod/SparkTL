...
for (j in size) {
  ...
  for(i in sleeping_times){
    ...
    df.sleeping$time_parallel <- df.sleeping$time_apply.mean + df.sleeping$time_instantiate.mean
    ...
    ref_value.time_parallel = ref_value$time_parallel
    ...
    df.sleeping["time_parallel.speedup"] =  ref_value.time_parallel / df.sleeping$time_parallel
    
    df.toprint = subset(df.sleeping, select = c(sleeping_time,element,link,executor,core,total_cores,partition,count,total_time.speedup,time_instantiate.speedup,time_apply.speedup,time_parallel.speedup))
    print(df.toprint)
...
```{r}
ggplot(data=df.with_speedups, aes(x=total_cores,y=time_parallel.speedup,group=as.factor(sleeping_time),color=as.factor(sleeping_time))) + 
...
ggsave("x2_sleeping_onlyparallel.pdf",width = 4, height = 4)
```
