...
object TransformationEngineOnePhase extends ExperimentalTransformationEngine{
    ...
    private def instantiateTraces_experiment[SME: ClassTag, SML, SMC, SMR, TME: ClassTag, TML: ClassTag]
    (tr: Transformation[SME, SML, SMC, TME, TML],  sm: Model[SME, SML], mm: Metamodel[SME, SML, SMC, SMR], npartition: Int,
     sc: SparkContext)
    : (List[TME], TraceLinks[SME, TME]) = {
        val tls : TraceLinks[SME, TME] = Trace.seq_trace_par_apply(tr, sm, mm, npartition, sc)
        (tls.getTargetElements , tls)
    }
    ...
    (tr: Transformation[SME, SML, SMC, TME, TML], sm: Model[SME, SML], mm: Metamodel[SME, SML, SMC, SMR],
     npartition: Int, sc: SparkContext): (Double, List[Double]) = {
        ...
        val elements_and_tls = instantiateTraces_experiment(tr, sm, mm, npartition, sc)
...
//        val links = applyTraces(tr, sm, mm, sps_rdd, tls_broad.value)
    ...
    def execute_with_collect_tuples[SME: ClassTag, SML, SMC, SMR, TME: ClassTag, TML: ClassTag]
    (tr: Transformation[SME, SML, SMC, TME, TML], sm: Model[SME, SML], mm: Metamodel[SME, SML, SMC, SMR],
     npartition: Int, sc: SparkContext): (Double, List[Double]) = {
        // 1.Instantiate the trace + output element
